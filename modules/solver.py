...
class TeachingSolver:
    def __init__(
        ...
        self.exclude_presenters_indeces = {
            self.person_to_index[person_name]
            for person_name in self.exclude_presenters
            if person_name in self.person_to_index.keys()
        }

        self.problem_solved = False
        self.problem = LpProblem(
            "learning-experiment-minimum-cost-schedule", LpMinimize
        )
        self.x = {
            (i, j, k): LpVariable(
                name=f"x[{i},{j},{k}]", cat=LpInteger, lowBound=0, upBound=1
            )
            for i in range(self.n_people)
            for j in self._people_minus({i}, self.exclude_presenters_indeces)
    ...
    def _add_constraints(self):
        ...
        for i in self.exclude_presenters_indeces:
            for k in range(self.n_blocks):
                self.problem += (
                    lpSum(
                        self.x[i, j, k]
                        for j in self._people_minus(
                            {i}, self.exclude_presenters_indeces
                        )
                    )
                    == 1
                )
        ...
        for k in range(self.n_blocks):
            ...
            self.problem += lpSum(
                ...
                for i in range(self.n_people)
                for j in self._people_minus({i}, self.exclude_presenters_indeces)
            ) >= floor((self.n_people - len(self.exclude_presenters)) / 2)

    def _get_priority_edges_for_listener(
        self, listener_idx, out_names, first_time=False
    ):
        ...
        for priority, presenter_name in enumerate(out_names):
            if presenter_name not in self.exclude_presenters:
                ...
                weight = (
                    ...
                    if not first_time
                    else self.weights[priority] ** first_time_multiplier
                )
    ...
    def _get_low_priority_edges_for_listener(self, listener_idx, out_names):
        ...
        listener_out_indeces = {
            self.person_to_index[person_name] for person_name in out_names
        }
        for presenter_idx in self._people_minus(listener_out_indeces, {listener_idx}):
            low_priority_edge_costs[
                listener_idx, presenter_idx
            ] = self.low_priority_weight
    ...
    def _get_edge_costs(self, data, first_time_people=[]):
        ...
        for listener in data:
            ...
            priority_edge_costs = self._get_priority_edges_for_listener(
                listener_idx, listener["out"], first_time
            )
            low_priority_edge_costs = self._get_low_priority_edges_for_listener(
                listener_idx, listener["out"]
            )
            edge_costs = {
                **edge_costs,
                **priority_edge_costs,
    ...
    def _add_objective(self, data, first_time_people=[]):
        ...
        self.problem += lpSum(
            self.x[i, j, k] * self.edge_costs[i, j]
            for i in range(self.n_people)
            for j in self._people_minus({i}, self.exclude_presenters_indeces)
            for k in range(self.n_blocks)
        )
        ...
        for key in self.x.keys():
            ...
            if int(value(self.x[i, j, k])):
                ...
                if presenter_name in results[k].keys():
                    results[k][presenter_name].append(str(listener_name))
                else:
                    results[k][presenter_name] = [str(listener_name)]

        out = {
            "results": [],
            "cost_achieved": value(self.problem.objective),
            "status": self.status,
        }
        for k in range(self.n_blocks):
            table_block = {"block": f"Teaching Block {k+1}", "presentations": []}
            for presenter, listeners in results[k].items():
                table_block["presentations"].append(
                    {"presenter": presenter, "listeners": listeners}
                )
            out["results"].append(table_block)
